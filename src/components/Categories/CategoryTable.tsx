...
export const CategoryTable = ({ isLoading, isError, filteredResults }: { isLoading: boolean, isError: boolean, filteredResults: Category[] }) => {
  return (
    <Table className={cn("block h-[24rem] border border-gray-500  min-w-lg rounded-sm overflow-y-scroll")}>
      <TableHeader className="sticky top-0 bg-[#ecefdc] shadow-sm w-full">
        <TableRow className="!w-full min-w-lg">
          <TableHead className="w-[200px]">Categoria</TableHead>
          <TableHead className="">Fecha de creación</TableHead>
          <TableHead className="w-[200px]">Última vez actualizada</TableHead>
        </TableRow>
      </TableHeader>
      ...
      <TableBody className={cn("", (isLoading || isError || filteredResults.length === 0) && "flex flex-col justify-center items-center h-[20rem]")}>
        {isLoading ? (
          ...
          filteredResults.map(category => (
            <TableRow className=" !border-gray-500 w-full" key={category.id}>
